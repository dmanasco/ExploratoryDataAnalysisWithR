...
```{r echo=FALSE, message=FALSE, warning=FALSE}
alc <- ggplot(aes(x = alcohol), data = white_wines) +
     geom_histogram(binwidth = .25, col="grey") +
     scale_x_continuous(breaks = 8:13)
fixedAcid <- ggplot(aes(x = fixed.acidity), data = white_wines) +
     geom_histogram(binwidth = .5, col="grey") +
     scale_x_continuous(breaks = 3.5:14.5)
volatileAcid <- ggplot(aes(x = volatile.acidity), data = white_wines) +
     geom_histogram(binwidth = .1, col="grey") +
     scale_x_continuous(breaks = 0:1.2)
citricAcid <- ggplot(aes(x = citric.acid), data = white_wines) +
     geom_histogram(binwidth = .1, col="grey") +
     scale_x_continuous(breaks = 0:1.8)
residualSugar <- ggplot(aes(x = residual.sugar), data = white_wines) +
     geom_histogram(binwidth = 1, col="grey")
chlorides <- ggplot(aes(x = chlorides), data = white_wines) +
     geom_histogram(binwidth = .05, col="grey")
sulfurDiox <- ggplot(aes(x = total.sulfur.dioxide), data = white_wines) +
     geom_histogram(binwidth = 10, col="grey")
freeSulfurDiox <- ggplot(aes(x = free.sulfur.dioxide), data = white_wines) +
     geom_histogram(binwidth = 10, col="grey")
densityWine <- ggplot(aes(x = density), data = white_wines) +
...
sulphates <- ggplot(aes(x = sulphates), data = white_wines) +
     geom_histogram(binwidth = .05, col="grey") +
     scale_x_continuous(breaks = 0.2:1.1)
grid.arrange(alc, fixedAcid, volatileAcid, citricAcid, residualSugar, chlorides, sulfurDiox, freeSulfurDiox,densityWine,ph,sulphates, ncol = 3)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
cleanedSugar <- white_wines[white_wines$residual.sugar < quantile(white_wines$residual.sugar, 0.99), ]
...
At this point the only variable I created is the bound sulfur dioxide variable. I created it taking the total sulfur dioxide and subtracting the free sulfur dioxide. I have only created a basic histogram for it, but I plan on using it during the bivariate analysis.
### Of the features you investigated, were there any unusual distributions? \
Did you perform any operations on the data to tidy, adjust, or change the form \
of the data? If so, why did you do this?
The only features that have an unuasual distribution was residual sugar. This data has a distribution with two peaks around 2 and 8 this leads me to believe there are two types of white wines in the dataset, those with higher sugar content and those with a lower sugar content. I did have to clean up the dataset with regards to residual sugar. There was a major outlier at 65. So i cleaned up the data and only included the data points with those that are lower than 95% quantile. After the data was cleaned up, it was easier to see the distribution. 
...
pair <- ggpairs(cleanWines,
  lower = list(continuous = wrap("points", color = "black", alpha = 0.1, size = 0.2))
)
pair <- pair + theme(
...
```{r echo=FALSE, warning=FALSE}
cleanSugarWines <- impWines[impWines$residual.sugar < quantile(impWines$residual.sugar, 0.99), ]
bigimppair <- ggpairs(cleanSugarWines,
  lower = list(continuous = wrap("points", color = "black", alpha = 0.3, size = .5))
)
p <- bigimppair[2,1] +
      geom_point(alpha= 1/10, position = 'jitter', color = 'yellow') +
...
```{r  echo=FALSE, warning=FALSE}
alcMean <- ggplot(aes(x=quality, y = alcohol_mean), data = ww.alc_by_qual, y) +
  geom_line(color = 'green') + ggtitle('Alcohol Mean vs Quality') + labs(x='Quality', y=' Alcohol Mean')
pHMean <- ggplot(aes(x=quality, y = pH_mean), data = ww.alc_by_qual) +
  geom_line(color = 'orange') + ggtitle('pH Mean vs Quality') + labs(x='Quality', y='pH Mean')
denMean <- ggplot(aes(x=quality, y = density_mean), data = ww.alc_by_qual) +
  geom_line(color = 'red') + ggtitle('Density Mean vs Quality') + labs(x='Quality', y='Density')
resMean <- ggplot(aes(x=quality, y = residualSugar_mean), data = ww.alc_by_qual) +
  geom_line(color = 'blue') + ggtitle('Residual Sugar Mean vs Quality') + labs(x='Quality', y='Residual Sugar Mean')
...
The strongest relationship I found was that between residual sugar and density. With an r value of about .89 this was the most correlated relationship. 
The Second most fascinating relationship that I found would have to be between alcohol and quality. With an r value of about .43 this relationship shows us that the higher quality wines also have higher alcohol content.
...
imppair_MV <- ggpairs(cleanSugarWines[,c(-6)], mapping = aes(color = quality),
                   lower = list(continuous = wrap("points", alpha = 0.1), combo = wrap("dot", alpha = 0.1)),
                   title = "White Wines"
                   )
...
p + ggtitle('Density vs Residual Sugar') + labs(x = 'Residual Sugar', y = 'Density')
```
